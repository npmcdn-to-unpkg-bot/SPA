Deployed site - 
https://single-page-app-f7cc1.firebaseapp.com



My SPA provides users a simple and intuitive web interface to record notes. 

In order to meet the requirements, I have included some sample notes [lorem ipsum] that are pulled via GET request from https://jsonplaceholder.typicode.com/todos.  

The web interface provides Create, Read, Update, Delete functionality at both the note level, and the global level. 

Additionally, notes can be marked with status 'Completed' or 'Not Completed'.

CRUD changes across notes to firebase with renderAll() on these triggers:
  
  1) Creating new or Editing: Save button in #detailsPanel, function submitTodo()

  2) Delete single: Delete button in each container of the items created via javascript, function deleteTodo(key)

  3) change completed value: Completed and Not completed buttons in each container of the items created via javascript, function changeCompleted(key, completed)

  4) function getFromServer(), for each retrieved item it posts to firebase

  5) deleteAll() and changeCompletedAll(true/false) functions



Hurdles:

Originally set out to build the app around 'beaches in AU'.  CRUD would be visited vs. not, and notes.  Couldn't find an API for beaches, looking into scraping HTML but couldn't pull in with AJAX request.  Ended up finding this notes API last minute.  

Another hurdle was to mark all the tasks as completed or not.
I had to update only 1 field for each task without changing other fields.
Firebase had no suitable function for this.
Had to get all the tasks from firebase first and update the field value by looping the results.

Completed and Not completed label.
While it's pretty easy to imagine how it works, it was not easy to find the best way to represent on front-end.
Create 2 different lists? or a checkbox? a label that changes color and status when you click on it?
Went with the last one.  

Edit an existing record.
What is the best way to fill the input with the existing title of the task?
There are many ways: passing as parameter, getting the data from firebase each time etc.
But the simplest i found is wrapping the title in a tag, give to the item's container unique id and get the title from the tag with document.querySelector('#todo-item-'+key+' .data-title').innerHTML
Same way for completed value.


Lessons learned:

Through this class the biggest takeaways for me are DOM manipulation and API calls/HTTP requests.  
